...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=wemaptech/multi-server:$SHA
kubectl set image deployments/client-deployment client=wemaptech/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=wemaptech/multi-worker:$SHA